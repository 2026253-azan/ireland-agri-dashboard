...
if "Data Table" in section or section == "🏷️ KPIs":
    ...
    # Attractive custom HTML table — no matplotlib needed
    def green_bar(val, mn, mx, color):
        pct = int((val - mn) / (mx - mn) * 100) if mx != mn else 0
        return (
            f'<div style="background:linear-gradient(to right,{color}33 {pct}%,'
            f'transparent {pct}%);padding:3px 8px;border-radius:4px;'
            f'font-weight:600;color:#1a2419">{int(val):,}</div>'
        )

    crops_mn,  crops_mx  = min(df["Crops (t)"]),    max(df["Crops (t)"])
    cattle_mn, cattle_mx = min(df["Cattle"]),        max(df["Cattle"])

    header = (
        '<div style="overflow-x:auto;border-radius:10px;border:1px solid #e4e9e2;margin-bottom:8px">'
        '<table style="width:100%;border-collapse:collapse;font-size:12.5px">'
        '<thead><tr style="background:#0d2b1a;color:white">'
        + "".join(
            f'<th style="padding:10px 12px;text-align:left;white-space:nowrap">{h}</th>'
            for h in ["Year","Crops (t)","Cattle","Sheep","Pigs","Chickens",
                      "Pesticides (t)","Pest kg/ha","Exports (t)","Imports (t)","Trade Balance"]
        )
        + '</tr></thead><tbody>'
    )
    ...
    for i, (_, r) in enumerate(df.iterrows()):
        ...
        bal_color = "#169b62" if r["Trade Balance"] >= 0 else "#c0392b"
        bal_sign  = "+" if r["Trade Balance"] >= 0 else ""
        body += (
            f'<tr style="background:{bg}">'
            f'<td style="font-weight:700;color:#169b62;padding:9px 12px">{int(r["Year"])}</td>'
            f'<td style="padding:6px 8px">{green_bar(r["Crops (t)"],  crops_mn,  crops_mx,  "#169b62")}</td>'
            f'<td style="padding:6px 8px">{green_bar(r["Cattle"],     cattle_mn, cattle_mx, "#003189")}</td>'
            f'<td style="padding:9px 12px;color:#444">{int(r["Sheep"]):,}</td>'
            f'<td style="padding:9px 12px;color:#444">{int(r["Pigs"]):,}</td>'
            f'<td style="padding:9px 12px;color:#444">{int(r["Chickens"]):,}</td>'
            f'<td style="padding:9px 12px;color:#444">{int(r["Pesticides (t)"]):,}</td>'
            f'<td style="padding:9px 12px;color:#444">{r["Pest Int (kg/ha)"]:.1f}</td>'
            f'<td style="padding:9px 12px;color:#444">{int(r["Exports (t)"]):,}</td>'
            f'<td style="padding:9px 12px;color:#444">{int(r["Imports (t)"]):,}</td>'
            f'<td style="padding:9px 12px;font-weight:700;color:{bal_color}">{bal_sign}{int(r["Trade Balance"]):,}</td>'
            f'</tr>'
        )

    footer = "</tbody></table></div>"
    st.markdown(header + body + footer, unsafe_allow_html=True)
    st.caption(f"Showing {len(df)} rows · Green bars show relative scale")
